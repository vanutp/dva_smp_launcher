    ...
    Callback: FnOnce() + Send + 'static,
{
    ...
    let versions_dir = modpacks_dir.join("versions");

    runtime.spawn(async move {
        let fut = get_complete_version_metadata(&version_info, &versions_dir);
        let metadata = tokio::select! {
            ...
            result = fut => match result {
                ...
                Err(e) => {
                    ...
                    let local_metadata = read_local_complete_version_metadata(&version_info, &versions_dir).await;
                    MetadataFetchResult {

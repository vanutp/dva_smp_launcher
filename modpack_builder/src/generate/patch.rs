...
use shared::{paths::{get_client_jar_path, get_libraries_dir}, version::version_metadata::VersionMetadata};

use crate::utils::get_url_from_path;

pub async fn replace_download_urls(version_name: &str, version_metadata: &mut VersionMetadata, download_server_base: &str, data_dir: &Path) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(library_path) = library.get_path(&get_libraries_dir(data_dir, version_name)) {
            if let Some(downloads) = &mut library.downloads {
                if let Some(artifact) = &mut downloads.artifact {
                    artifact.url = get_url_from_path(&library_path, data_dir, download_server_base)?;
                }
            }

            if let Some(url) = &mut library.url {
        ...
        if let Some(downloads) = &library.downloads {
            if let Some(natives) = &downloads.classifiers {
                for (natives_name, download) in natives.clone() {
                    ...
                    let natives_path = library.get_natives_path(&natives_name, &download, &libraries_dir);
                    new_natives_urls.push(get_url_from_path(&natives_path, data_dir, download_server_base)?);
                }
            }
        }

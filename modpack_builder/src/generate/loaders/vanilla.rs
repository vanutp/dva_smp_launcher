...
use log::info;
use reqwest::Client;
use shared::{files::download_file, paths::get_versions_dir, version::{version_manifest::{fetch_version_manifest, VersionInfo}, version_metadata::{get_version_metadata_path, read_version_metadata, save_version_metadata, VersionMetadata}}};
...
impl VanillaGenerator {
    pub fn new(version_name: String, minecraft_version: String, download_server_base: String, replace_download_urls: bool) -> Self {
        Self {
            version_name,
            minecraft_version,
...
pub async fn download_vanilla_metadata(version_info: &VersionInfo, output_dir: &Path) -> Result<VersionMetadata, Box<dyn Error + Send + Sync>> {
...
impl VersionGenerator for VanillaGenerator {
    async fn generate(&self, output_dir: &Path, _: &Path) -> Result<(), Box<dyn Error + Send + Sync>> {
        info!("Generating vanilla version {}, minecraft version {}", self.version_name, self.minecraft_version);

        let vanilla_manifest = fetch_version_manifest(VANILLA_MANIFEST_URL).await?;
        let version_info = vanilla_manifest.versions.iter().find(|v| v.id == self.minecraft_version).ok_or_else(|| {
            VanillaGeneratorError::VersionNotFound
        })?;
        ...
        if self.replace_download_urls {
            ...
            replace_download_urls(&self.version_name, &mut vanilla_metadata, &self.download_server_base, output_dir).await?;
            let versions_dir = get_versions_dir(&output_dir);
